...
/*
    ...
    Descomente o console.log abaixo e veja o resultado no console.

    Por que será que a função não está substituindo o caractere do index 3 da string?
...
/* 
    ...
    const multiplicar = (a, b, c) => {
        if (ehUndefined(a) || ehUndefined(b) || ehUndefined(c)) {
...
const somar = (arg1, arg2, arg3) => {
    ...
    const temSoArg1E2 = !ehUndefined(arg1) && !ehUndefined(arg2) && ehUndefined(arg3);
    const temTodosArgs = !ehUndefined(arg1) && !ehUndefined(arg2) && !ehUndefined(arg3);
    const temNenhumArg = [arg1, arg2, arg3].every(ehUndefined);
...
const dobrar = (numeros) => {
    const numerosDobrados = [];
    numeros.forEach((item) => numerosDobrados.push(item * 2));
    return numerosDobrados;
};
...
const saoMesmaVersao = (obj) => {
    return obj.versoes.every((versao) => versao[0] === versaoOficial);
};
...
let jsPresenteNosItens = arquivos.every((item) => item.includes('js'));
...
const arrays = (array1, array2) => [...array1, ...array2];
const bandas = arrays(bandasAnos60, bandasAnos70);
